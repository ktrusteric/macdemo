...
const demoUsers = [
  {
    ...
    energy_types: ["天然气"],  // 覆盖率最高：42.2% (19篇文章)
  ...
  },
  {
    ...
    energy_types: ["原油"],  // 覆盖率最高：42.2% (19篇文章)
    expected_articles: 19
  },
  {
    ...
    energy_types: ["液化天然气(LNG)"],  // 第三高：24.4% (11篇文章)
  ...
  },
  {
    ...
    energy_types: ["管道天然气(PNG)"],  // 第四高：22.2% (10篇文章)
...
| 能源类型 | 覆盖文章数 | 覆盖率 | 对应用户 |
|---------|-----------|--------|---------|
| 天然气 | 19篇 | 42.2% | 张工程师（上海） |
| 原油 | 19篇 | 42.2% | 李经理（北京） |
| 液化天然气(LNG) | 11篇 | 24.4% | 王主任（深圳） |
| 管道天然气(PNG) | 10篇 | 22.2% | 陈总监（广州） |
| 电力 | 4篇 | 8.9% | 刘研究员（成都） |
